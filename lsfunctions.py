# lsfunctions.py - version v1.59 - 03-December 2025 - Removed hardcoded password.
# calculating domain to use and storing in dom variable
# testing tcp port 443 to detect pfSense.
# bug fixes and updates to manage linux services
# bug fix to correctly disable the Firefox reset banner with exception handling
# added sendlabhealth function especially at lab fail
# changes due to labstartup modularization
# better error handling and retry in start_nested
# bug ejecting idisk and cdrom for Toolbox AutoCheck
# Conky color change from green to red for any fail status
# option to write_output to specify logfile - useful for labupdater and dolabupdate
# added user_pref("browser.disableResetPrompt", true); to user.prefs file to silence Firefox reset message
# exception handling in check_datastore
# updates to managevcsaservice for better logging
# multiple updates to ssh, scp, read_file_into_list, report_vpodstatus
# and managevcsaservice now working properly with pfSense and Debian
# added FAIL to labstartup_sleep message. Need that to prevent run on.
# remove labcheck cron on normal run and pass labcheck argument to startup scripts
# added blocking to ssh function
# significant update to all subprocess calls to use the run() method with blocking, capture plain text stdout/stdout and returncode
# update for Manager VM to make certain that "FAIL" is included in the /hol/startup_status.txt file
# improved error handling for check_datastore
# added set_status_color function to directly set the color of the Conky desktop status line
# backing out manager stuff since this will always run on the LMC
# not calling labhealth if vPod fails
# fix for pyVmomi 8.0.1.0 special case HOL-2440 only at this point.
# removing the manager variable and updating get_all_hosts() to deal with HCX
# fixed test_pings to wait in the loop if initial ping fails
# change usmask to 0002 for 775 (rwxrwxr-x) 08/23/2023
# don't update Conky if running labcheck 08/23/2023
# determine domain using proxy DNS record and not IP address
# correcting location of mouseaction to keep labcheck from running 8/30/2023
# replacing plink and pscp for sshpass for simpler password handling and no sshkeys management 12/27/2023
# many, many changes implementing for 2024 dev cycle. 05/15/2024
# update to start nested. instead of only waiting for task, check the powerstate and break the loop. 08/25/2024
# update check_datastore to reboot hosts if NFS mount is inaccessible 11/19/2024
# added reboot_hosts function for NFS mount issues. checking accessible in start_nested 11/21/2024
...
if os.path.isfile(configini):
    ...
    if 'domain' in config['VPOD'].keys():
        dom = config.get('VPOD', 'domain')

if dom == 'NOTSET':
    # dom could be corp.local or corp.vmbeans.com or vcf.sddc.lab so calculate it
    # the socket.getfqdn() is no longer returning a fqdn but hostname -A does.
    try:
        res = subprocess.run(['/usr/bin/nslookup', 'manager'], capture_output=True, text=True, check=True)
        lines = res.stdout.split('\n')
        for line in lines:
            if 'manager' in line:
                (junk, fqdn) = line.split('\t')
    except Exception as e:
        res = subprocess.run(['/usr/bin/hostname', '-A'], capture_output=True, text=True, check=True)
...
single_password = ''
vpod_path = f'{mcholroot}/vPod.txt'

if os.path.exists(vpod_path):
    with open(vpod_path, 'r') as f:
        for line in f:
            if line.startswith('password='):
                single_password = line.strip().split('=')[1]
                break
...
# this dictionary can most likely be deleted
status_table = {
    'STARTING': 1,
    'GOOD-2': 2,
    'GOOD-3': 3,
    ...
    'LC-GOOD-9': 59,
    'LC-GOOD-10': 60,
    'LC-TIMEOUT': 199,  # timer exceeded => too long to go READY is a FAIL (added 5/6/2021 for consistency)
    'LABCHECK': 99,  # post-READY check-up for stale pods.
    'FAIL-1': 101,
    'FAIL-2': 102,
    'FAIL-3': 103,
    'FAIL-4': 104,
    ...
    'FAIL-9': 109,
    'FAIL-10': 110,
    'ODYSSEY-FAIL': 120,  # pod has passed tests and is ready for use
    'TIMEOUT': 199,  # timer exceeded => too long to go READY is a FAIL
    'READY': 200,  # pod has passed tests and is ready for use
    'AUTOLAB': 201,  # used for scale test automation (pod is consumed)
    'AUTOCHECK': 202,  # used for DEV validation of pods (pod is consumed)
    'TIMEOUT-OLD': 203,  # deprecated - do not use (placeholder until all VMworld 2018 pods are gone)
    'FIREWALL': 204,  # thrown if pod is wired but the FW is not on. Potentially non-fatal, but not secure
    'ALERT': 205,  # non-fatal warning of some kind
    'AUTOCHECK-DONE': 206,  # AutoCheck process has completed and logs are ready for harvest
    'ODYSSEY-READY': 220  # Odyssey client is present
}
...
def init(**kwargs):
    ...
    **kwargs: logfile to use
    ...
    if 'password' in config['VPOD'].keys():
        password = config['VPOD']['password']
        rtrpassword = password
    else:
        password = f'{single_password}{single_password}'
        rtrpassword = password
    if 'vcenterpass' in config['VPOD'].keys():
       vcenterpass = config['VPOD']['vcenterpass'].split('\n')
    ...
    if 'odyssey' in config['VPOD'].keys():
        if config.get('VPOD', 'odyssey').casefold() == 'true'.casefold():
            odyssey = True
    write_output('===== Detected Password: single_password: {single_password}, password: {password}, rtrpassword: {rtrpassword} =====')
    ...
    if chkrouter:
        ...
        while True:
            ...
            res = subprocess.run(['/usr/bin/nslookup', f'proxy.{dom}'], 
                capture_output=True, text=True, check=True)
            if res.returncode != 0:
                write_output(f'Waiting for DNS to resolve proxy.{dom} - pause 30 seconds...')
                labstartup_sleep(30)
            else:
                write_output(f'proxy.{dom} resolves in DNS now.')
                break
        # wait for the proxy to start
        while not test_tcp_port(f'proxy.{dom}', 3128, logfile=lfile):
            write_output(f'Waiting for proxy on proxy.{dom}...', logfile=lfile)
            labstartup_sleep(sleep_seconds)

        # wait for the router to be ready - date command removed
...
def test_ping(host):
    ...
    while not res == 0:
        ...
        labstartup_sleep(sleep_seconds)
        res = subprocess.Popen(['ping', '-c', '3', host], stdout=subprocess.DEVNULL).wait()
    write_output(f'Successfully pinged {host}')
    return True
...
def test_labcheck():
    ...
    """
    # check for mouse activity - DISABLING THIS - DOES NOT WORK - NEED VLPAGENT  
    if WMC and labcheck:
        # run a wincmd on the WMC to determine last input       
        command = f'pwsh C:\\Users\\Administrator\\mousewatch.ps1 {labcheckinterval} > C:\\Users\\Administrator\\mousewatch.log'
        result = runwincmd(command, 'mainconsole', 'Administrator', password, logfile='labstartup.log')
    """
...
def connect_vcenters(entries):
    ...
    for entry in entries:
        ...
        test_ping(vc[0])
        if vc_type == 'windows':
            # TODO: start vpxd service on a Windows machine?
            pass
        if vc_type == 'esx':
            # vc_restarted = True
            login_user = 'root'
        ...
        if type == 'windows':
            write_output("vSphere web client start NOT IMPLEMENTED on windows vCenter.")
        elif type == 'linux':
            write_output('Starting vsphere-ui')
            run_command(f'ssh root@{vc[0]}.{dom} service-control --start vsphere-ui')


def connect_restart_vcenters(entries):
    """
    attempt to connect to all vCenters passed in the list entries
    :param: entries: list of vCenter entries from /hol/Resources/vCenters.txt
    """
    global sis
    global start_time
    ...
    for entry in entries:
        vc = entry.split(':')
        write_output(f'Connecting to {vc[0]}...')
        vc_type = vc[1]
        if len(vc) == 3:
            login_user = vc[2]
        # wait for ping
        ...
        while not connect_vc(vc[0], login_user, password):
            labstartup_sleep(sleep_seconds)
            elapsed = datetime.datetime.now() - vc_start_time
            if elapsed > vc_boot_minutes:
                if not vc_restarted:
                    vc_restarted = True
                    write_output(f'Restarting vCenter {vc[0]}')
                    if restart_vc(entry):
                        vc_start_time = datetime.datetime.now()
                        max_minutes_before_fail = max_minutes_before_fail + (vc_boot_minutes.total_seconds() / 60)
                        # don't think we should reset the start time since we've added vc_boot_minutes
                else:
                    # TODO: write_vpod_progress fail the lab
                    write_output('Cannot restart ' + vc[0] + ' Failing the lab.')
            elapsed = datetime.datetime.now() - vc_start_time
            if elapsed > vc_boot_minutes and vc_restarted:
                write_output('Failing the lab after restarting vCenter ' + vc[0])
        if type == 'windows':
            write_output("vSphere web client start NOT IMPLEMENTED on windows vCenter.")
        elif type == 'linux':
            write_output('Starting vsphere-client')
            run_command(f'ssh root@{vc[0]}.{dom} service-control --start vsphere-ui')
    return max_minutes_before_fail
...
def restart_vc(entry):
    """
    restart the vCenter listed in the vCenter entry from /hol/Resources/vCenters.txt
    param: entry: the entry string from vCenters.txt (: delimited)
    """
    vc = entry.split(':')
    vc_type = vc[1]
    if vc_type == 'windows':
        write_output('restart for Windows vCenter NOT IMPLEMENTED!')
        return False
    # reboot PSC first if present
    psc = f'psc-01a.{dom}'
    if test_ping(psc):
        write_output('Trying Platform Services Contoller reboot...')
        res = run_command('ssh root@' + psc + ' init 6')
        if hasattr(res, 'stdout'):
            write_output(res.stdout)
        labstartup_sleep(60)
        while not test_ping(psc):
            labstartup_sleep(60)
    # now reboot vCenter appliance
    res = run_command('ssh root@' + vc[0] + ' init 6')
    if hasattr(res, 'stdout'):
        write_output(res.stdout)
    return True
...
def get_all_objs(si_content, vimtype):
    ...
    """
    obj = {}
    if hasattr(si_content, 'viewManager'):
...
def get_cluster(cs_name):
    ...
    for cluster in clusters:
        if hasattr(cluster, 'name') and cluster.name == cs_name:
            return cluster
...
def enable_drs(cs):
    ...
    try:
        if hasattr(cs, 'ReconfigureComputeResource_Task'):
            task = cs.ReconfigureComputeResource_Task(cluster_spec, True)
            WaitForTask(task)
            return True
        return False
    except Exception as e:
        write_output(f'Exception: {e} {cs.name if hasattr(cs, "name") else ""}' + str(e))
        return False
...
def get_datastore(ds_name):
    ...
    for si in sis:
        ...
        for datastore in datastores:
            if hasattr(datastore, 'name') and datastore.name == ds_name:
                return datastore
...
def get_all_hosts():
    ...
    for si in sis:
        ...
        # noinspection PyTypeChecker
        for host in hosts:
            if hasattr(host, 'summary') and 'VMware Mobility Agent' not in host.summary.hardware.model:
...
def get_host(name):
    ...
    for si in sis:
        hosts = get_all_objs(si.content, [vim.HostSystem])
        for host in hosts:
            if hasattr(host, 'name') and host.name == name:
                return host
...
def get_vm(name):
    ...
    for si in sis:
        vms = get_all_objs(si.content, [vim.VirtualMachine])
        for vm in vms:
            if hasattr(vm, 'name') and vm.name == name:
                return vm
...
def get_vapp(name):
    ...
    for si in sis:
        ...
        for vapp in vapps:
            if hasattr(vapp, 'name') and vapp.name == name:
                return vapp
...
def check_datastore(entry):
    ...
    while True:
        try:
            ...
            if ds and hasattr(ds, 'summary') and (ds.summary.type == 'VMFS' or 'NFS' in ds.summary.type or ds.summary.type == 'vsan'):
                break
        except Exception as e:
            write_output(f'Exception checking datastores on {server}')
            write_output(f'Exception: {e}', logfile='/tmp/lsterrors.log')
        if ds and hasattr(ds, 'summary'):
            print(ds.summary.type)
        labstartup_sleep(sleep_seconds)

    if ds and hasattr(ds, 'summary') and ds.summary.type == 'VMFS':
        hosts = []
        try:
            hosts = get_all_hosts()
        except Exception as e:
            ...
            for host in hosts:
                ...
                while True:
                    try:
                        ...
                        if hasattr(e, 'args') and e.args and 'Licenses' in e.args[0]:
                            ...
                            write_output(f'Exception: {e}', logfile='/tmp/lsterrors.log')
                    labstartup_sleep(sleep_seconds)
    elif ds and hasattr(ds, 'summary') and 'NFS' in ds.summary.type:
...
def start_nested(records):
    ...
    for record in records:
        ...
        for vm in vms:
            if va:
                good_power = 'started'
                if hasattr(va, 'summary') and va.summary.vAppState == good_power:
                ...
                if hasattr(vm, 'runtime') and vm.runtime.powerState == good_power:
            ...
            while True:
                ...
                if task:
                    ...
                    if e_type == 'vApp':
                        while True:
                            va = get_vapp(e_name)
                            if va and hasattr(va, 'summary') and va.summary.vAppState == good_power:
                                write_output(f'{va.name} is powered on.')
                                break
                            else:
                                write_output(f'Waiting for {va.name if va else "vApp"} to power on. ...')
                                if hasattr(task, 'info') and task.info.error:
...
def get_network_adatper(vm_obj):
   """
...
def labstartup_sleep(seconds):
    ...
    if delta.seconds >= (max_minutes_before_fail * 60):
        message = f'Lab startup has run for the maximum minutes of {max_minutes_before_fail} and has been terminated.'
...
def addroute(nw, nm, gw):
    ...
    if hasattr(res, 'stdout') and res.stdout and nw in res.stdout:
    ...
    with open(lf, "w") as f:
        f.write(rtecmd)
        f.close()
    scp(lf, f'root@router.{dom}:{rf}', pw)
...
def runwincmd(cmd, server, user='Administrator', pw=None, **kwargs):
    """
    ...
    try:
        ...
        try:
            ...
            return f'fail: {e}'
        out = f'{cmd}: {out}'
        write_output(out, logfile=lfile)
        if display:
            print(out)
        try:
            c.remove_service()
...
def start_autocheck():
    ...
    if os.path.exists(thescript):
        ...
        if prepare_idisk():
            ...
            ssh('/usr/bin/umount /mnt/idisk', f'root@mainconsole.{dom}', password)
            write_output('iDisk unmounted.')
            # eject the CD
            write_output('Ejecting the CD...')
            ssh('/usr/bin/eject', f'root@mainconsole.{dom}', password)
...
def router_finished():
    ...
    ret = ssh(rcmd, f'holuser@router.{dom}', rtrpassword)
    if hasattr(ret, 'stdout') and 'finished' in ret.stdout:
        return True
    else:
        return False
...
def startup(fname):
    ...
    if 'FAIL' in status[0].upper():
        if hasattr(run, 'stderr'):
            write_output(f'{fname}output: {run.stdout} {run.stderr}')
        else:
            write_output(f'{fname}output: {run.stdout}')
        kill_labstartup()
        exit(1)
    if hasattr(run, 'returncode') and run.returncode != 0:
